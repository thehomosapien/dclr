...
The RPM spec file provided here is for DCLR-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://dclr.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DCLR git repository](https://github.com/DCLRProject/DCLRcoin) but are not part of
...
Currently only `Source10` is used, for the example `dclr.conf` file.
...
in the dclr git repository. At present only an SVG version of the DCLR
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the dclr
...
needed when 0.12.1 is released, a proper fix is already in the DCLR git
master branch.
...
idea to build DCLRcoin Core against a static version of that library compiled
...
The `dclr.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `dclr-server` package. At this point, most RPM based
...
The `dclr-server` package should have SELinux support. How to properly do
...
DCLR-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/dclr/). He has been
packaging DCLR for Fedora at least since 2012.
...
4. I renamed the `dclr` package that contains the Qt GUI to `dclr-core` as
that appears to be how the general population refers to it, in contrast to
`dclr-xt` or `dclr-classic`. I wanted to make sure the general population
