...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** DCLR START */
    case OP_DCLR_ASSET              : return "OP_DCLR_ASSET";
    /** DCLR END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_DCLR_ASSET) { // OP_DCLR_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == DCLR_R) { // Check to see if DCLR starts at 27 ( this->size() < 105)
                if ((*this)[28] == DCLR_V)
                    if ((*this)[29] == DCLR_N)
                        index = 30;
            } else {
                if ((*this)[28] == DCLR_R) // Check to see if DCLR starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == DCLR_V)
                        if ((*this)[30] == DCLR_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == DCLR_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == DCLR_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == DCLR_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == DCLR_R) {
