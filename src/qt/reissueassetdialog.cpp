...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (asset->nAmount + ui->quantitySpinBox->value() * COIN > MAX_MONEY) {
        showMessage(tr("Quantity is too large. Max is 81,700,000"));
        return;
    }
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid DCLR Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " DCLR";
    amount.append("</b>");
