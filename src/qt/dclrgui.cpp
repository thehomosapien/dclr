...
const std::string DCLRGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString DCLRGUI::DEFAULT_WALLET = "~Default";

DCLRGUI::DCLRGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void DCLRGUI::createActions()
{
    ...
    overviewAction = new QAction(platformStyle->SingleColorIconOnOff(":/icons/overview_selected", ":/icons/overview"), tr("&Dashboard"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a DCLR address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and dclr: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to DCLR addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your DCLR addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified DCLR addresses"));
    ...
    openAction->setStatusTip(tr("Open a dclr: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible DCLR command-line options").arg(tr(PACKAGE_NAME)));
...
//        toolbar->addAction(votingAction);
        ...
        /** DCLR Vertical Sidebar START */
        QString tbStyleSheet = ".QToolBar {background-color : transparent; border-color: transparent; }  "
                               ".QToolButton {background-color: transparent; margin-bottom: 35px; border-color: transparent; width: 249px; color: #2ed7a3; border: none;} "
                               ".QToolButton:checked {background: none; background-color: none; selection-background-color: none; color: #2ed7a3; border: none; font: %4} "
                               ".QToolButton:hover {background: none; background-color: none; border: none; color: white;} "
        ...
        QVBoxLayout* dclrLabelLayout = new QVBoxLayout(toolbarWidget);
        dclrLabelLayout->addWidget(label);
        dclrLabelLayout->addWidget(toolbar);
        dclrLabelLayout->setDirection(QBoxLayout::TopToBottom);
        dclrLabelLayout->addStretch(1);
...
void DCLRGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool DCLRGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool DCLRGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void DCLRGUI::setWalletActionsEnabled(bool enabled)
{
...
void DCLRGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void DCLRGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void DCLRGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DCLRGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void DCLRGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void DCLRGUI::openClicked()
{
...
void DCLRGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void DCLRGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void DCLRGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void DCLRGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void DCLRGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void DCLRGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** DCLR START */
void DCLRGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void DCLRGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void DCLRGUI::gotoManageAssetsPage()
{
...
void DCLRGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to DCLR network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void DCLRGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void DCLRGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void DCLRGUI::updateHeadersSyncProgressLabel()
{
...
void DCLRGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void DCLRGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("DCLR"); // default title
...
void DCLRGUI::changeEvent(QEvent *e)
{
...
void DCLRGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DCLRGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void DCLRGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "DCLR")
        msg += tr("Amount: %1\n").arg(DCLRUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(DCLRUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void DCLRGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DCLRGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool DCLRGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool DCLRGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void DCLRGUI::setHDStatus(int hdEnabled)
{
...
void DCLRGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void DCLRGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void DCLRGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DCLRGUI::detectShutdown()
{
...
void DCLRGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DCLRGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void DCLRGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(DCLRGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void DCLRGUI::subscribeToCoreSignals()
{
...
void DCLRGUI::unsubscribeFromCoreSignals()
{
...
void DCLRGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<DCLRUnits::Unit> units = DCLRUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const DCLRUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(DCLRUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (DCLRUnits::Unit u : DCLRUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(DCLRUnits::name(u)), this);
...
{
    setText(DCLRUnits::name(newUnits));
}
...
void DCLRGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=DCLRBTC"));
