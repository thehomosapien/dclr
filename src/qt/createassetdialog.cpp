...
void CreateAssetDialog::setUpValues()
{
    ...
    QStringList list;
    list.append(tr("Main Asset") + " (" + DCLRUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::ROOT)) + ")");
    list.append(tr("Sub Asset") + " (" + DCLRUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB)) + ")");
    list.append(tr("Unique Asset") + " (" + DCLRUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::UNIQUE)) + ")");
...
void CreateAssetDialog::setupCoinControlFrame(const PlatformStyle *platformStyle)
{
    ...
    ui->labelCoinControlQuantityText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlAmountText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlAmountText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlFeeText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlFeeText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlAfterFeeText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlAfterFeeText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlBytesText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlBytesText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlLowOutputText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlLowOutputText->setFont(GUIUtil::getSubLabelFontBolded());

    ui->labelCoinControlChangeText->setStyleSheet(STRING_LABEL_COLOR);
    ui->labelCoinControlChangeText->setFont(GUIUtil::getSubLabelFontBolded());
...
void CreateAssetDialog::setupAssetDataView(const PlatformStyle *platformStyle)
{
    ...
    ui->assetTypeLabel->setFont(GUIUtil::getSubLabelFontBolded());

    ui->assetNameLabel->setStyleSheet(STRING_LABEL_COLOR);
    ui->assetNameLabel->setFont(GUIUtil::getSubLabelFontBolded());

    ui->addressLabel->setStyleSheet(STRING_LABEL_COLOR);
    ui->addressLabel->setFont(GUIUtil::getSubLabelFontBolded());

    ui->quantityLabel->setStyleSheet(STRING_LABEL_COLOR);
    ui->quantityLabel->setFont(GUIUtil::getSubLabelFontBolded());

    ui->unitsLabel->setStyleSheet(STRING_LABEL_COLOR);
    ui->unitsLabel->setFont(GUIUtil::getSubLabelFontBolded());
...
void CreateAssetDialog::setupFeeControl(const PlatformStyle *platformStyle)
{
    ...
    ui->createAssetButton->setFont(GUIUtil::getSubLabelFontBolded());
...
void CreateAssetDialog::CheckFormState()
{
    ...
    if (!(ui->addressText->text().isEmpty() || IsValidDestination(dest)) && assetNameValid) {
        ...
        showMessage(tr("Warning: Invalid DCLR address"));
        return;
    }
...
void CreateAssetDialog::onCreateAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetBurnAmount(type), 8)) + " DCLR";
    amount.append("</b>");
    ...
    if(nFeeRequired > 0)
    {
        ...
        questionString.append(DCLRUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nFeeRequired));
