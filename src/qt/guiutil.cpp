...
#include "dclraddressvalidator.h"
#include "dclrunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DCLR address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new DCLRAddressEntryValidator(parent));
    widget->setCheckValidator(new DCLRAddressCheckValidator(parent));
}
...
bool parseDCLRURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no dclr: URI
    if(!uri.isValid() || uri.scheme() != QString("dclr"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DCLRUnits::parse(DCLRUnits::DCLR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDCLRURI(QString uri, SendCoinsRecipient *out)
{
    // Convert dclr:// to dclr:
    //
    //    Cannot handle this later, because dclr:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dclr://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "dclr:");
    }
    QUrl uriInstance(uri);
    return parseDCLRURI(uriInstance, out);
}

QString formatDCLRURI(const SendCoinsRecipient &info)
{
    QString ret = QString("dclr:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DCLRUnits::format(DCLRUnits::DCLR, info.amount, false, DCLRUnits::separatorNever));
        paramCount++;
    }
...
bool openDCLRConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(DCLR_CONF_FILENAME);
    ...
    /* Open dclr.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DCLR.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DCLR (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("DCLR (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "dclr.desktop";
    return GetAutostartDir() / strprintf("dclr-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a dclr.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=DCLR\n";
        else
            optionFile << strprintf("Name=DCLR (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the dclr app
...
bool GetStartOnSystemStartup()
{
    CFURLRef dclrAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (dclrAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, dclrAppUrl);

    CFRelease(dclrAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add dclr app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, dclrAppUrl, nullptr, nullptr);
    }
