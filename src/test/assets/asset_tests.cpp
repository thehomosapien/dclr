...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of DCLRCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("DCLR", type));
        BOOST_CHECK(!IsAssetNameValid("DCLR", type));
        BOOST_CHECK(!IsAssetNameValid("DCLRCOIN", type));

        //- Versions of DCLRCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("DCLR.COIN", type));
        BOOST_CHECK(IsAssetNameValid("DCLR_COIN", type));
        BOOST_CHECK(IsAssetNameValid("DCLRSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERDCLR", type));
        BOOST_CHECK(IsAssetNameValid("DCLRSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEDCLR", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_DCLRS", type));
        BOOST_CHECK(IsAssetNameValid("SEDCLROT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("DCLR", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
